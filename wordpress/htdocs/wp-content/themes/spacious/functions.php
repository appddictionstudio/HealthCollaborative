 ...
 * Enqueue scripts
 */
add_action( 'wp_enqueue_scripts', 'hc_load_javascript' );

function hc_load_javascript() {
  ...
  wp_register_script( 'program-tabs', get_template_directory_uri() . '/js/program-tabs.js', array('jquery'), '1.0', true );
  wp_enqueue_script( 'program-tabs' );
}
...
/**
 * Override Spacious Footer Copyright
 */
function spacious_footer_copyright() {
  $site_link = '<a href="' . esc_url( home_url( '/' ) ) . '" title="' . esc_attr( get_bloginfo( 'name', 'display' ) ) . '" ><span>' . get_bloginfo( 'name', 'display' ) . '</span></a>';

  $as_link = '<a href="'.esc_url( 'http://appddictionstudio.com' ).'" target="_blank" title="' . esc_attr__( 'Appddiction Studio', 'spacious' ) . '"><span>' . __( 'Appddiction Studio', 'spacious' ) . '</span></a>';

  $default_footer_value = sprintf( __( 'Copyright &copy; %1$s %2$s.', 'spacious' ), date( 'Y' ), $site_link ).' '.sprintf( __( 'Powered by %s.', 'spacious' ), $as_link );
