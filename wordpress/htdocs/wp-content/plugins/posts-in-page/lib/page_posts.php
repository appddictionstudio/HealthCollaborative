...
class ICPagePosts {
	...
	protected function paginate_links( $posts ){
		...
		$prev = ( $curr_page && $curr_page > 1 ) ? '<li><a href="'.$page_url.'?page='. ( $curr_page-1 ).'" class="previous-posts">Previous</a></li>' : '';
		$next = ( $curr_page && $curr_page < $total_pages ) ? '<li><a href="'.$page_url.'?page='. ( $curr_page+1 ).'" class="next-posts">Next</a></li>' : '';
